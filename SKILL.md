...
Verify `success` is `true`. If the coin is unspent, this errors — check `spent` via `get_coin_record_by_name` first. If `--inspect` fails, retry without it.
...
If `coin_records` is empty, verify the parent coin exists with `get_coin_record_by_name` first — it may be unspent or the ID may be incorrect.
...
See [references/conditions.md](references/conditions.md) for the full opcode table (CREATE_COIN, AGG_SIG_*, announcements, messages, self-assertions, timelocks, concurrency).
...
See [references/puzzle-hashes.md](references/puzzle-hashes.md) for the full table of well-known puzzle template hashes (standard transaction, CAT v2, singleton, settlement, DID, NFT layers, TAILs) with curried args.
